...
export default function ConcreteNCPage() {
  ...
  const filteredNCs = ncs.filter(nc =>
    ...
    nc.vendor_name?.toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Concrete Non-Conformance (NC)</h1>
        <p className="text-gray-600">Track concrete quality issues and vendor performance</p>
      </div>
      ...
      <div className="bg-white rounded-lg border p-4 mb-6">
        <div className="flex flex-col md:flex-row gap-4">
          <div className="flex-1 relative">
            ...
            <input
              type="text"
              placeholder="Search by NC number, description, vendor..."
              value={searchQuery}
          ...
          <Link
            href="/dashboard/concrete-nc/new"
        ...
        {filteredNCs.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">
              {searchQuery ? 'Try adjusting your search' : 'No concrete NCs raised yet'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Batch/Test</th>
                ...
                {filteredNCs.map((nc) => (
                  <tr key={nc.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      ...
                      <div className="text-sm text-gray-500">{nc.issue_type}</div>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-900">{nc.vendor_name}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">
                      {nc.batch_number || nc.test_id || 'N/A'}
                    </td>
                    ...
                    <td className="px-6 py-4 text-right">
                      <Link
                        href={`/dashboard/concrete-nc/${nc.id}`}
